-- MZ TAILOR - PERBAIKAN DATABASE YANG SUDAH PERNAH DISETUP
-- Gunakan file ini jika setup.sql gagal dengan:
-- ERROR 42710: type "order_status" already exists
-- Jalankan seluruh file ini sekali di Supabase SQL Editor.

-- Pengamanan: order hanya dibuat oleh admin terautentikasi.
...
-- Foto referensi hanya boleh diunggah admin.
...
-- Pastikan akun Auth pertama admin dan akun berikutnya staff.
...
begin
  ...
  values(new.id,coalesce(new.raw_user_meta_data->>'full_name',split_part(new.email,'@',1)),case when not exists(select 1 from public.profiles where role='admin') then 'admin' else 'staff' end);
  return new;
end; $$;
